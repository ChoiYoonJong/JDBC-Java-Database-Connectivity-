...
public class MemberController {// view 와 dao 를 연결해주는 객체
	...
	public void selectAll() { 
		...
		ArrayList<Member> list = new MemberService().selectAll();//Dao 에 있는 selectAll 호출
	...
	public void selectOne(String memberId) { //멤버 아이디를 받아왔다. //하나만 갖고 와서 조회하기 때문에 ArrayList가 필요하지 않는다.
		MemberMenu menu = new MemberMenu(); // 메뉴로..
		Member m = new MemberService().selectOne(memberId); // selectOne(member)를 MemberDAO 에 넘겨서 조회를 한다.
	...
	public void selectByName(String memberName) {
		...
		List<Member> list = new MemberService().selectByName(memberName); // 여러 몇의 이름을 조회 할 수 있기때문에 list 생성
	...
	public void insertMember(Member m) {
		int result = new MemberService().inserMember(m); // 쿼리가 재대로 수행되면 쿼리문에 insert,update,delete 문은 쿼리문에 성공 갯수가 리턴이 된다. 그래서 insert 가 몇개가 성공이 됬는지 리터이 되기때문에 반화해서 넘겨준다.		
	...
	public void updateMember(Member m) {//매개 변수를 m 으로 받는다.
		int result = new MemberService().updateMember(m); // 쿼리가 재대로 수행되면 쿼리문에 insert,update,delete 문은 쿼리문에 성공 갯수가 리턴이 된다. 그래서 insert 가 몇개가 성공이 됬는지 리터이 되기때문에 반화해서 넘겨준다.		
	...
	public void selectAllDeleteMember() {
		MemberMenu menu = new MemberMenu(); // 메뉴로.. // view 생성
		List<Member> list = new MemberService().selectAllDeleteMember(); // 탈퇴 전부 조회
